...
const CartItem: React.FC<VoucherComponentProps> = ({
    ...
    const handleQuantityChange = async (productVariantId: number, quantity: number, historyQuantity: number) => {
        const updateQuantity = historyQuantity + quantity;
        try {
            const rte = await updateCartApi({username: "", productVariantId: productVariantId, quantity: updateQuantity});
                        ...
                        <span>{attr.value}</span>
                            ...
                            <td className="py-4">
                                <div className="flex items-center w-1/2 rounded-full  border-gray-200 bg-white">
                                    <button
                                        onClick={() => handleQuantityChange(cart.productVariantDTO.productVariantId, -1, cart.quantity)}
