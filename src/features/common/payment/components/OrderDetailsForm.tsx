...
export const OrderDetailsForm = () => {
    ...
    const handleReview = async (e: React.MouseEvent<HTMLButtonElement>, orderItemId: number) => {
        e.preventDefault();
            ...
            <span className="text-gray-700 text-2xl font-medium">
              ...
              <span className="text-gray-700 text-2xl font-medium">
                Trạng thái
                            ...
                            <span className="text-gray-700 text-2xl font-medium">
                {orderDetails?.status === "PENDING" ? "Đang chờ xử lý" : orderDetails?.status === "SHIPPING" ? "Đang giao hàng" : orderDetails?.status === "COMPLETED" ? "Đã giao hàng" : "Đã hủy"}
              </span>
                        </div>
