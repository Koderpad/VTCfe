...
// Find the lowest and highest prices among product variants
    const minPrice = Math.min(...product.productVariantDTOs.map((variant) => variant.price));
    const maxPrice = Math.max(...product.productVariantDTOs.map((variant) => variant.price));
    ...
    return (
        <section className="lg:pl-20">
          ...
          {selectedVariantID_ForAddToCart
              ...
              : `Từ ${formatPrice(minPrice)} VNĐ đến ${formatPrice(maxPrice)} VNĐ`}
        </span>
                    </p>
